...
try:
    ...
    ALLOW_DIRECT_FALLBACK = getattr(settings, 'ALLOW_DIRECT_FALLBACK', True)
    if not SCRAPERAPI_KEY:
        logger.warning("⚠️ settings.py 中未找到 SCRAPERAPI_KEY，将仅使用直连模式")
except ImportError:
    logger.warning("⚠️ 未找到 settings.py 文件，将仅使用直连模式")
    SCRAPERAPI_KEY = ""
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 DataFetcher V23.5 (Settings Integrated)")
    print("=" * 60)
