...
def calculate_position_v13(tech, ai_adj, ai_decision, val_mult, val_desc, base_amt, max_daily, pos, strategy_type, fund_name):
    ...
    if tactical_mult > 0:
        ...
        if val_mult >= 1.5 and strategy_type in ['core', 'dividend']: final_mult = 0.5; reasons.append(f"战略:左侧定投")
    if cro_signal == "VETO" and final_mult > 0: final_mult = 0; reasons.append(f"🛡️风控:否决")
    held_days = pos.get('held_days', 999)
    if final_mult < 0 and pos['shares'] > 0 and held_days < 7: final_mult = 0; reasons.append(f"规则:锁仓({held_days}天)")
...
def process_single_fund(fund, config, fetcher, tracker, val_engine, analyst, market_context, base_amt, max_daily):
    ...
    try:
        ...
        val_mult, val_desc = val_engine.get_valuation_status(fund.get('index_name'), fund.get('strategy_type'), fund.get('code'))
        ...
        return {
            "name": fund['name'], "code": fund['code'], "index_name": fund.get('index_name'), 
            "amount": amt, "sell_value": s_val, "is_sell": is_sell, "tech": tech, "ai_analysis": ai_res
        }, cio_log, []
    except Exception as e:
...
def main():
    ...
    if TEST_MODE and funds: funds = funds[:1]
    results, cio_lines = [], []
    logger.info("🚀 启动处理...")
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        for f in as_completed(futures):
            res, log, _ = f.result()
            if res: results.append(res); cio_lines.append(log); print(f"✅ 完成处理: {res['name']}") 
    if results:
    ...
    else: logger.warning("⚠️ 没有生成任何结果")
