...
def calculate_position_v13(tech, ai_adj, ai_decision, val_mult, val_desc, base_amt, max_daily, pos, strategy_type, fund_name):
    """
    V13.1 核心算分逻辑 (适配 v3.5 HOLD_CASH)
    """
    ...
    if ai_decision == "REJECT": 
        ...
        tactical_score = 0 # 强制归零，触发卖出/空仓逻辑
    elif ai_decision == "HOLD" and tactical_score >= 60: 
        tactical_score = 59 # 压分，保持观望
...
def process_single_fund(fund, config, fetcher, tracker, val_engine, analyst, market_context, base_amt, max_daily):
    ...
    try:
        ...
        return {
            ...
            "decision": lbl,
            "tech": tech, 
            "ai_full": ai_res  # 【🔥关键修复】必须用 ai_full，与 ui_renderer.py 保持一致
        }, cio_log, []
    except Exception as e:
...
def main():
    ...
    if analyst:
        ...
        vitality = scanner.get_market_vitality() # 获取资金流
        market_context = {
    ...
    if results:
        ...
        send_email(f"{subject_prefix}鹊知风 V19.1 全量化仪表盘", html) 
